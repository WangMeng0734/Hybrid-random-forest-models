...
num_res = size(res, 1);                   
num_size = 0.8;                          
flag_conusion = 1;                        The % flag is 1, turning on the confusion matrix
...
lb  = [10, 01];                    % Optimization parameter target lower limit
ub  = [500, 06];                   % Optimization parameter target upper limit
pop = 100;                          % Number of sparrows
Max_iteration = 500;               % The maximum number of iterations

%%  optimization
[Best_pos, Best_score, curve, avcurve] = SSA(pop, Max_iteration, lb, ub, dim, fun);
...
disp('-------------------------------------------------------------')
disp('SSA-RF Training set error index')
[confmat,Accuracy,Precision,Recall,F1_score]=calc_error(T_train, T_sim1);
fprintf('\n')
disp('-------------------------------------------------------------')
disp('SSA-RF Test set error index')
